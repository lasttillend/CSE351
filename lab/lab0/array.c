...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer: array does not have enough space to contain more
  //         than 10 elements so it is an out of bounds access
  ...
  // Answer: because the function fillArray expects a pointer
  //         to an int as the first argument and &value has that
  //         type.
  fillArray(&value, 1);
  ...
  fillArray(&(heap_fourInts->c), 1);
  fillArray(&(heap_fourInts->d), 1);
