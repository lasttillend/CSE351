...
int thirdBits(void) {
  /*
   * Desired output k :     01 001 | 001 001 001 | 001 001 001 | 001 001 001
   * Magic number 73  =                                          001 001 001
   * 73 << 9          =                            001 001 001 | 000 000 000
   * 73 << 9 | 73     =                            001 001 001 | 001 001 001
   * Then shift the above number (which denotes by x) to the left by 9 again
   * x << 9           =              001 001 001 | 001 001 001 | 000 000 000
   * x << 9 | 73      =              001 001 001 | 001 001 001 | 001 001 001
   * Finally shift the above number (which denotes by y) to the left by 6
   * and add 001 001 (which is 9) to get the final result 
   * y << 6 + 9       = 0 | 01 001 | 001 001 001 | 001 001 001 | 001 001 001
   */
  int x = (73 << 9) | 73;
  int y = (x << 9) | 73;
  return (y << 6) + 9;
...
int addOK(int x, int y) {
    int sum = x + y;
    int mask = 1;
    int signbit_x = (x >> 31) & mask;
    int signbit_y = (y >> 31) & mask;
    int signbit_sum = (sum >> 31) & mask ;
    // Overflow occurs when
    // (1) x > 0, y > 0 but sum <= 0, or
    // (2) x < 0, y < 0 but sum >= 0
    // In terms of signbits,
    // (1) 0, 0, 1 
    // (2) 1, 1, 0
 ...
 */
int bang(int x) {
    // If x != 0, then one of x and -x will be positive and the other one negative,
    // thus their sign bits will be 1 and 0 (or vice versa). Right shift with 31
    // results in 0xFFFFFFFF and 0. Then take OR operation we will still get 0xFFFFFFFF,
    // which is -1. Finally, add 1 leads to 0, which is what we want i.e., any
    // nonzero value should be evaluated to 0.
...
int conditional(int x, int y, int z) {
    // Check https://stackoverflow.com/questions/10034470/conditional-statement-using-bitwise-operators 
    int a = !!x;
    int b = ~a + 1; 
    return (b & y) | (~b & z);
}
...
int isPower2(int x) {
    int minus_one = ~0;
    int is_positive = (!!x) & (!(x >> 31));
    return is_positive & !(x & (x + minus_one));
}
